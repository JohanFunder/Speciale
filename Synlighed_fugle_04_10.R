...
directory <- "G:/Vadehavet/GIS_projekts/Arcgis_pro/Forlandskantforsøg/Forlandskantforsøg_24_10/Relativ_synlighed_fugle"

# For droneoverflyvning 04_10

Relativ_synlighed <- file.path(directory,"/RS_fugle_04_10.csv")
Relativ_synlighed
Relativ_synlighed <- read.csv(file=Relativ_synlighed, header= TRUE)
str(Relativ_synlighed)

#Lav vektor med rastervaules

rela_04_10 <- Relativ_synlighed$RASTERVALU

gen_04_10 <- mean((Relativ_synlighed$RASTERVALU),na.rm=TRUE)
median((Relativ_synlighed$RASTERVALU),na.rm=TRUE)
head(rela_04_10)

# Lav scatterplot over den relative synlighed
par(mfrow=c(1,1))
...
## For droneoverflyvning 04_10 med NNjoin

Relativ_synlighed <- file.path(directory,"/RS_fugle_04_10_NNjoin.csv")
Relativ_synlighed
Relativ_synlighed <- read.csv(file=Relativ_synlighed, header= TRUE)
str(Relativ_synlighed)
...
rela_04_10 <- Relativ_synlighed$RASTERVALU
rela_04_10_NNjoin <- Relativ_synlighed$distance

gen_04_10 <- mean((Relativ_synlighed$RASTERVALU),na.rm=TRUE)
median((Relativ_synlighed$RASTERVALU),na.rm=TRUE)
head(Relativ_synlighed)

# Lav scatterplot over den relative synlighed

plot(rela_04_10_NNjoin,rela_04_10,xlim = c(18,65),ylim = c(0,2))

mtext("Distance from edge of foreland", side=1, cex=1.3, line=0.5, outer=TRUE)   
mtext("Relative visibility (m)", side=2, cex=1.3, line=1.5, outer=TRUE)
mtext("Relative visibility of waterbirds in the blind zone", side=3, cex=1.3, line=-0.8, outer=TRUE)
...
bar <- barplot(gen_23_10, xlab = "Silhuet højde", ylab = "Tillægshøjde i m for synlighed" ,
               main = "Simulering af synligheden for forsøg 23. okt.",col=c("grey"))
...
Relativ_synlighed <- file.path(directory,"/Relativ_synlighed_24_10.csv")
Relativ_synlighed
Relativ_synlighed <- read.csv(file=Relativ_synlighed, header= TRUE)
str(Relativ_synlighed)
...
# Samlet barplot()
par(mfrow=c(2,1))
barplot(gen_23_10, ylab = "Tillægshøjde i m for synlighed" ,
